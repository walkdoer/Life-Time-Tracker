    ...
    importProject(log.project, _.pick(log, ['tags', 'classes', 'start', 'end']))
      .then(function (projectId) {
        ...
        importVersion(version, {
            ...
            //import task;
            importTask(log.task, log.subTask, {
                projectId: projectId,
                versionId: versionId,
                createTime: log.start,
                lastActiveTime: log.end
...
function importTask(taskObj, subTaskObj, info) {
    ...
    if (!taskObj) {
        ...
        Task.findOne(condition, function (err, task) {
            ...
            if (task) {
                ...
                if (subTaskObj) {
                    subTask = task.subTasks.filter(function(doc) {
                        return doc.name === subTaskObj.name;
                    })[0];
                    if (!subTask) {
                        task.subTasks.push(appendInfo(subTaskObj));
                    } else {
                        subTask.attributes = extend(subTask.attributes, subTaskObj.attributes);
                        subTask.progress = subTaskObj.progress;
                        subTask.save();
                    }
                }
                task.save(function (err, result, numberAffected) {
                    ...
                    var lastIndex = result.subTasks.length - 1;
                    if (!subTask && subTaskObj) { //means new task, then take the last sub task, that would be the newest one
                        subTask = result.subTasks[lastIndex];
                    }
                    deferred.resolve({
                        task: result,
                        subTask: subTask
                ...
                if (subTaskObj) {
                    taskObj.subTasks = [appendInfo(subTaskObj)];
                }
