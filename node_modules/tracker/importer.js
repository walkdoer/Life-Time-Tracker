...
function importLog(date, log) {
    ...
    Q.allSettled(promises).then(function () {
        ...
        return importPeoples(log.peoples);
    }).then(function () {
        //import project
...
function importPeoples (peoples) {
    if (!peoples) {
        return Q(1);
    }
    ...
    return peoples.reduce(function (promise, people) {
        return promise.then(importPeople(people));
    }, Q(1));
}
...
function importPeople(people) {
    var deferred = Q.defer();
    People.count({_id: people}, function (err, count) {
        if (err) {
            return deferred.reject(err);
        }
        if (count === 0) {
            People.create({_id: people, lastActiveTime: new Date()}, function (err) {
                if (err) {
                    return deferred.reject(err);
                }
                console.log('Import people ' + people + ' success');
