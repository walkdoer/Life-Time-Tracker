...
function importLog(date, log) {
    ...
    //importProject
    importProject(log.project, log.start).then(function (projectId) {
        //importVersion
        var version = null;
        if (log.project && log.project.version) {
            version = {name: log.project.version};
        }
        importVersion(version, projectId).then(function (versionId) {
            //import task;
            importTask(log.task, projectId, versionId).then(function (taskId) {
                //if have subTask, then save the subtask with parent task Id
                if (log.subTask) {
                    log.subTask.parent = taskId;
                    importTask(log.subTask, projectId).then(function (subTaskId) {
                        saveLog({
                            project: projectId,
                            task: subTaskId,
                            version: versionId
...
function importProject(project, start, end) {
    var deferred = Q.defer();
    if (!project) {
    ...
    Project.findOne(queryCondition, function (err, result) {
        ...
        if (!result) {
            project.createdTime = new Moment(start).toDate();
            project.lastActiveTime = new Moment(end).toDate();
            ...
            result.lastActiveTime = new Moment(end).toDate();
            result.save(function (err, project, numberAffected) {
                Msg.debug('Project ' + project.name + ' exists, last active time updated, affected' + numberAffected + 'record');
...
function importTask(taskObj, projectId, versionId) {
    var deferred = Q.defer();
    if (!taskObj) {
        deferred.resolve(null);
    } else {
...
function importVersion(versionObj, projectId) {
    var deferred = Q.defer();
    if (!versionObj) {
        deferred.resolve(null);
    } else {
        var condition = {name: versionObj.name};
        Version.findOne(condition, function (err, version) {
            ...
            if (version) {
                Msg.debug('Task ' + versionObj.name + ' exist ' + version.id);
                deferred.resolve(version.id);
            } else {
                versionObj.projectId = projectId;
                ...
                versionModel.save(function (err, result) {
                    ...
                    Msg.debug('Successfully import version ' + name + ' _id:' + result.id);
                    deferred.resolve(result.id);
                });
            }
        });
