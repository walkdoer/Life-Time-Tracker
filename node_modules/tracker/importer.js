...
function importDay(day) {
    ...
    Log.remove({
        ...
        t2 = Date.now();
        console.log('remove log cost: ' + (t2 - t1));
        if (err) {
        ...
        logs.reduce(function(promise, log) {
            ...
            t3 = Date.now();
            console.log('import log cost:' + (t3 - t2));
            return statAndSaveToDatabase(day);
        }).then(function () {
            t4 = Date.now();
            console.log('save and stat log cost:' + (t4 - t3));
            deferred.resolve();
...
function statAndSaveToDatabase(day) {
    ...
    Stat.findOne({date: new Moment(day.date).toDate()}, function (err, stat) {
        ...
        if (!stat) {
            console.log('create stat result' + day.date);
            Stat.create({
            ...
            console.log('update stat result' + day.date);
            stat.result = result;
    ...
    Q.allSettled(promises).then(function () {
        ...
        importVersion(version, {
            ...
            importTask(log.task, {
                ...
                //console.log(log.task, log.subTask);
                ...
                function saveLog(refer) {
                    ...
                    logModel.save(function(err, log) {
                        ...
                        if (importedLogCount === waitToImportedLogCount) {
                            console.log('Import Logs finished, count:' + importedLogCount);
        ...
        if (!result) {
            ...
            if (_.isNumber(project.progress)) {
                if (diffLastActiveTime >= 0) {
                    console.log('update progress of ' + result.name + ' to ' + project.progress);
                    result.progress = project.progress;
                } else {
                     console.log('no need to update progress, of ' + project.name + ' diff=' + diffLastActiveTime);
                }
            }
            if (diffLastActiveTime >= 0) {
            ...
            Project.update({_id: result._id}, result, function (err, row) {
                if (err) {
                    Msg.error('Project' + ' update failed!' + JSON.stringify(project), err);
                    deferred.reject(err);
                    return;
                }
                Msg.debug('Project ' + result.name + ' exists, last active time updated, affected' + row+ 'record');
                deferred.resolve(result.id);
            });
        }
    });
        ...
        Task.findOne(condition, function (err, task) {
            ...
            if (task) {
                ...
                if (_.isNumber(taskObj.progress)) {
                    if (diffLastActiveTime >= 0) {
                        console.log('update progress of ' + task.name + ' to ' + taskObj.progress + ' diff:' + diffLastActiveTime);
                        task.progress = taskObj.progress;
                    } else {
                        console.log('no need to update progress, of ' + task.name + ' diff=' + diffLastActiveTime);
                    }
                    if (taskObj.progress === 100 && lastActiveTime) {
    ...
    if (!versionObj) {
        ...
        Version.findOne(condition, function (err, version) {
            ...
            if (version) {
                ...
                extend(version.attributes, versionObj.attributes);
                var diffLastActiveTime = new Moment(lastActiveTime).diff(version.lastActiveTime);
                if (_.isNumber(versionObj.progress)) {
                    if (diffLastActiveTime >= 0) {
                        console.log('update progress of ' + version.name + ' to ' + versionObj.progress + ' diff:' + diffLastActiveTime);
                        version.progress = versionObj.progress;
                    } else {
                        console.log('no need to update progress, of version ' + version.name);
                    }
                ...
                if (diffLastActiveTime >= 0) {
                    ...
                    console.log('no need to update lastActiveTime of version ' + version.name);
                }
                ...
                if (versionObj.progress === 100 && lastActiveTime) {
                    ...
                    console.log('complete version ' + JSON.stringify(versionObj));
                }
