...
function importProject(project, attrs) {
    ...
    Project.findOne(queryCondition, function (err, result) {
        ...
        if (!result) {
            ...
            //update progress
            if (_.isNumber(project.progress) && diffLastActiveTime >= 0) { //if has progress and progress need to
                if (diffLastActiveTime >= 0) {
                    console.log('update progress of ' + result.name + ' to ' + project.progress);
                    result.progress = project.progress;
                } else {
                     console.log('no need to update progress, of ' + project.name + ' time:' + attrs.end + ' origin: '
                       + result.lastActiveTime + ' diffLastActiveTime=' + diffLastActiveTime);
                }
            }
            if (diffLastActiveTime >= 0) {
    ...
    if (!taskObj) {
        ...
        Task.findOne(condition, function (err, task) {
            ...
            if (task) {
                ...
                console.log(task.name + ' || new: ' + lastActiveTime + ' old:' + task.lastActiveTime);
    ...
    if (!versionObj) {
        ...
        Version.findOne(condition, function (err, version) {
            ...
            if (version) {
                ...
                var diffLastActiveTime = new Moment(options.lastActiveTime).diff(version.lastActiveTime);
                if (diffLastActiveTime >= 0) {
