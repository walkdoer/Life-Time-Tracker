...
function importLog(date, log) {
    ...
    Q.allSettled(promises).then(function () {
        ...
        return importPeoples(log.peoples, log.end || log.start);
    }).then(function () {
        //import project
        ...
        //if no version, then put in Inbox
        if (!version) {
            version = {name: 'Inbox'};
        }
...
function importPeoples (peoples, lastActiveTime) {
    if (!peoples) {
        return Q(1);
    }
    ...
    return peoples.reduce(function (promise, people) {
        var relations = peoples.filter(function (p) { return p !== people});
        return promise.then(importPeople(people, lastActiveTime, relations));
    }, Q(1));
}


function importPeople(people, lastActiveTime, relations) {
    var deferred = Q.defer();
    if (!_.isArray(relations) && _.isString(relations)) {
        relations = [relations];
    }
    People.findById(people, function (err, result) {
        if (err) {
            return deferred.reject(err);
        }
        if (!result) {
            People.create({
                _id: people,
                relations: relations,
                lastActiveTime: new Moment(lastActiveTime).toDate()
            }, function (err, result) {
                if (err) {
                    return deferred.reject(err);
                }
                Msg.success('Import people ' + people + ' success');
                deferred.resolve(result);
            });
        } else {
            var diffLastActiveTime = new Moment(lastActiveTime).diff(result.lastActiveTime);
            if (diffLastActiveTime > 0) {
                result.lastActiveTime = new Moment(lastActiveTime).toDate();
            }
            if (result.relations && !_.isEmpty(relations)) {
                console.log('addToSet', relations);
                result.relations.addToSet(relations);
            }
            result.save(function (err, res, row) {
                if (err) {
                    Msg.error('People' + people + ' update failed!', err);
                    deferred.reject(err);
                    return;
                }
                Msg.debug('People ' + people + ' exists, last active time updated, affected' + row+ 'record');
