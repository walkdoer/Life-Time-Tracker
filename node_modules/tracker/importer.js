...
function importPeoples (peoples, lastActiveTime) {
    ...
    return peoples.reduce(function (promise, people) {
        return promise.then(importPeople(people, lastActiveTime));
    }, Q(1));
}


function importPeople(people, lastActiveTime) {
    var deferred = Q.defer();
    People.findById(people, function (err, result) {
        ...
        if (!result) {
            People.create({_id: people, lastActiveTime: new Moment(lastActiveTime).toDate()}, function (err, result) {
                if (err) {
                    return deferred.reject(err);
                }
