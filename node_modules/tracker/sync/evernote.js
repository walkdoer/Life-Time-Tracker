...
exports.backUp = function (days, options) {
    ...
    options = _.extend({
        versionPrefix: new Date().getTime().toString()
    }, options);
    var noteBookName = new moment().format('YYYYMMDDHHmmss');
    //create nootBook and backup all files to this notebook
    createNoteBook(noteBookName).then(function (createdNoteBook) {
        Msg.success('Successfully created a new notebook '+ noteBookName + ' with GUID' + createdNoteBook.guid);
        Q.settleAll(days.map(function (day) {
            var title = new moment(day.date).format('YYYY-MM-DD');
            return createNote(title, day.origin);
        })).then(function (results) {
            var failedNotes = [];
            results.forEach(function (res) {
                if (res.state === 'rejected'){
                    failedNotes.push(res.value);
                }
            });
            deferred.resolve({success: true, failedNotes: failedNotes});
        });
    }).catch(function (err) {
...
function createNoteBook(name) {
    ...
    notebook.name = name;
    userStore.createNotebook(authToken, notebook, function (err, notebook) {
        if (err) {
            deferred.reject(err);
            return;
        }
...
function createNote(title, content, notebookGuid) {
    ...
    noteStore.createNote(note, function (err, createdNote) {
        ...
        Msg.success('Successfully created a new note with GUID' + createdNote.guid);
