...
exports.backup = function (days, options) {
    ...
    createNotebook(notebookName).then(function (createdNoteBook) {
        ...
        Q.allSettled(days.map(function (day) {
            var title = new moment(day.date).format('YYYY-MM-DD');
            return createNote(title, day.content, notebookGuid);
 ...
 */
function createNote(title, content, notebookGuid) {
    var deferred = Q.defer();
    var note = new Evernote.Note();
    note.title = title;
...
exports.backUpLogFile = function (date, logContent) {
    ...
    if (!eventLogNoteBookGuid) {
        console.error('no book guid');
        promise = findNoteBook(EVENT_NOTEBOOK_NAME).then(function (notebook) {
            if (!notebook) {
                return deferred.reject(new Error(EVENT_NOTEBOOK_NAME +  ' notebook does not exist.'));
            }
            console.error('find book guid' + notebook.guid);
            eventLogNoteBookGuid = notebook.guid;
            return findNote(date, eventLogNoteBookGuid);
        }).then(function (note) {
            return saveNote(note);
        });
    } else {
        promise = findNote(date, eventLogNoteBookGuid).then(function (note) {
            return saveNote(note);
        });
    }
    ...
    function saveNote(note) {
        ...
        if (note) {
            ...
            promise = createNote(date, logContent, eventLogNoteBookGuid);
        }
