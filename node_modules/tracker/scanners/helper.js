...
function readLogFile(options) {
    ...
    if (options.files) {
        options.files.forEach(function (fileName) {
            readQueue.push(logFileHelper.readLogFiles(fileName));
        });
        ...
        while (mStart.diff(mEnd) < 0) {
            ...
            readQueue.push(logFileHelper.readLogFiles(date));
            mStart.add(1, 'days');
        }
...
function readOneDayLog(year, month, day) {
    ...
    return logFileHelper.readLogFiles(date);
}
...
function readLogByDateRange(from, to) {
    ...
    while (fromMoment.diff(toMoment) < 0) {
        ...
        queue.push(logFileHelper.readLogFiles(date));
