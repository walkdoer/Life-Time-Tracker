...
var serverPort = require('../conf/config').serverPort;
...
app.post('/importLogContent', function (req, res) {
    var body = req.body;
    Importer.importLogContent(body.date, body.logContent).then(function () {
        res.status(200).send({success: true});
    }).catch(function (err) {
        res.status(507).send({success: false, err: err});
    });
...
var confFilePosition = path.resolve(__dirname, '../conf/userSettings.json');


app.get('/settings', function (req, res) {
    fs.readFile(confFilePosition, function (err, data) {
        if (err) {
...
app.post('/settings', function (req, res) {
    fs.writeFile(confFilePosition, JSON.stringify(req.body,  null, 4), function (err) {
        if (err) {
