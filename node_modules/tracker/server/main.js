...
var SearchPeople = require('../search/searchPeople');
...
var searchHelper = require('../search/searchHelper');
var AchieveGoal = require('./AchieveGoal');
...
var Routine = require('../search/Routine');
...
app.get('/taskCountTrend', function (req, res) {
    var query = req.query;
    var dateFormat = "%Y-%m-%d";
    var condition = {}, completeTime = null;
    var filterDone = query.done === 'true';
    var list = query.list === 'true';
    switch (query.group) {
        case 'year':
            dateFormat = '%Y';
            break;
    ...
    if (query.start) {
        completeTime = {};
        completeTime['$gte'] = new Moment(query.start).toDate();
    }
    if (query.projectId) {
        condition.projectId = new ObjectId(query.projectId);
    }
    if (query.end) {
        completeTime = completeTime || {};
        completeTime['$lt'] = new Moment(query.end).toDate();
    }
    if (completeTime) {
        condition.completeTime = completeTime;
    }
    if (filterDone) {
    ...
    var groupOptions = {
        _id: {
            $dateToString: { format: dateFormat, date: filterDone ? "$completeTime" : '$lastActiveTime'}
        },
        count: { $sum: 1 }
    };
    if (list) {
        groupOptions.tasks =  { $push: "$$ROOT" };
    }
    Task.aggregate({
        $match: condition
    })
...
app.post('/tasks/:id', function (req, res) {
    ...
    if (taskId) {
        ...
        data.updateTime = new Moment().toDate();
    ...
    if (taskId) {
        Task.findByIdAndRemove(taskId, function (err, result) {
            ...
            if (result) {
                ...
                res.status(404).send({success: false, message: 'task not exist', err: 'task not exist'});
            }
        });
    } else {
    ...
    if (versionId) {
        Version.findByIdAndRemove(versionId, function (err, result) {
            ...
            if (result) {
                ...
                return res.status(404).send({success: false, err: 'version not exist'});
            }
        });
    } else {
        res.status(500).send({success: false, err: 'need version id'});
    }
});
...
app.delete('/projects/:id', function (req, res) {
    ...
    if (projectId) {
        ...
        res.status(500).send({success: false, message: 'need project id'});
    }
});
...
app.get('/importByDate', function (req, res) {
    var query = req.query, start, end;
    if (query.start && query.end) {
        start = new Moment(query.start).startOf('day').toDate();
        end =  new Moment(query.end).endOf('day').toDate();
    } else if (query.date) {
        start = new Moment(query.date).startOf('day').toDate();
        end =  new Moment(query.date).endOf('day').toDate();
    }
    Importer.importFromLogFile({
        start: start,
        end: end
    }).then(function () {
        res.status(200).send({success: true});
...
get('countTasks', null, countTasks);
get('tags', null, queryTags);
get('peoples', null, queryPeoples);
get('logs', '/:year?/:month?/:day?', queryLogs);
get('classifyLogs', '/:year?/:month?/:day?', classifyLogs);
...
get('routine', null, function (params) {
    return Routine.find(params);
});
get('achieveGoal', null, function (params) {
    return AchieveGoal.achieve(params);
});
    ...
    if (goalId) {
        Goal.findByIdAndRemove(goalId, function (err, result) {
            ...
            if (result) {
                ...
                res.status(404).send({success: false, message: id + ' not exist.'});
            }
        });
    } else {
        res.status(500).send({success: false, message: 'need goal id'});
    }
});
...
app.get('/settings', function (req, res) {
    ...
    if(fs.existsSync(userSettingsFilePath)) {
        userSettings = JSON.parse(fs.readFileSync(userSettingsFilePath));
    }
...
app.post('/settings', function (req, res) {
    fs.writeFile(userSettingsFilePath, JSON.stringify(req.body,  null, 4), function (err) {
        ...
        /*var timer = setTimeout(function () {
            if (config.isChanged('serverPort')) {
                exports.stop().then(function () {
                    console.log('restart server...');
                    return exports.start();
                }).then(function () {
                    console.log('restart success in port ' + config.get('serverPort'));
                });
            }
            clearTimeout(timer);
        }, 1000);*/
...
app.get('/classes', function (req, res) {
    LogClass.find().sort('-_id').exec(function (err, data) {
        res.status(200).send(data);
    });
});


app.get('/wakeAndSleep', function (req, res) {
    var query = req.query;
    var matchCondition = {
        type: {$in: [constant.WAKE_LOG, constant.SLEEP_LOG]}
    };
    if (query.start && query.end) {
        var start = new Moment(query.start).startOf('day').toDate(),
            end =  new Moment(query.end).endOf('day').toDate();
        matchCondition.date = {
            $gte: start,
            $lte: end
        ...
        return res.status(500).send({message: "need start and end to specify date range"});
    }*/
    var agg = Log.aggregate({$match: matchCondition});
    if (query.group) {
        var groupParams = searchHelper.getGroupParams(query.group);
        agg = agg.group({
            _id: groupParams,
            moment: {$push: {start: "$start", date: "$date"}}
        });
    }
    agg.exec(function (err, result) {
        ...
        var data = {};
        try {
            data.wake = result.filter(function (val) {
                return val._id == constant.WAKE_LOG;
            })[0].moment;
            data.sleep = result.filter(function (val) {
                return val._id == constant.SLEEP_LOG;
            })[0].moment;
            res.status(200).send(data);
        } catch(e) {
...
app.delete('/classes/:id', function (req, res) {
    ...
    var id = params.id;
    if (id) {
        LogClass.findByIdAndRemove(id, function (err, result) {
            if (err) {
                Msg.error('delete log class failed!', err);
                return res.status(500).send({success: false, err: err});
            }
            if (result) {
                res.status(200).send(result);
        ...
        res.status(500).send({success: false, err: 'need log class id'});
    }
});
...
app.post('/classes/:id', function (req, res) {
    ...
    if (id) {
        var data = req.body;
        LogClass.findByIdAndUpdate(id, data, function (err, logClass) {
            if (err) {
                Msg.error('update logClass failed', err);
                return res.status(507).send({success: false, err: err});
            }
            res.status(200).send(logClass);
        });
    } else {
        return res.status(507).send({success: false, message: 'need class id'});
    }
});


app.get('/sleepLength', function (req, res) {
    var query = req.query;
    var queryConditions = {};
    if (query.start && query.end) {
        queryConditions.date = {
            $gte: Moment(query.start).startOf('day').toDate(),
            $lt: Moment(query.end).endOf('day').toDate()
        };

        Stat.find(queryConditions, "result.date result.sleepTime").sort({date: 1}).exec(function (err, results) {
            if (err) {
                return res.status(500).send(err);
            }
            var data;
            if (query.sum === 'true' || query.mean === 'true') {
                var total  = results.reduce(function (sum, day){
                    var statResult = day.result;
                    var sleepTime = statResult.sleepTime || 0;
        ...
        res.status(500).send({message: 'do not have start and end params'});
    }
});


get('stat', null, function (options) {
    var deferred = Q.defer();
...
function queryPeoples(params) {
    return SearchPeople.query(params);
}
...
function countTasks(params) {
    return SearchTask.count(params);
}
...
function classifyLogs(params) {
    return Search.classify(params);
}
...
exports.start = function (options) {
    var deferred = Q.defer();
    options = _.extend({port: config.get('serverPort')}, options);
    if (!server) {
