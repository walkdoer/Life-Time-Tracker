...
exports.focus = function (options, scanResult) {
    ...
    statResult.sleepTime = calculateSleepTime(date, wakeMoment);
...
function calculateSleepTime(date, wakeMoment) {
    var timeSpan = -1;
    /**
    ...
    var yesterdayLogs = getYesterDayLogs(date);
    if (!_.isEmpty(yesterdayLogs)) {
        var yesterdaySleepMoment = helper.getSleepMoment(yesterdayLogs);
        timeSpan = helper.timeSpan(yesterdaySleepMoment, wakeMoment);
    }
    return timeSpan;
    function getYesterDayLogs(date) {
        var nd = prevDay(date);
        var file;
        try {
...
function prevDay(date) {
    return new Moment(date).subtract(1, 'days').format(dateFormat);
}