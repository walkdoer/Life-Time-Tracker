...
exports.classify = function (options) {
    var deferred = Q.defer();
    options = _.extend({
        populate: true
    }, options);
    ...
    aggregate.exec(function (err, result) {
        ...
        Project.populate(result, [{path: '_id', select: 'name _id'}]).then(function (result) {
            Q.allSettled(result.map(function (project) {
                if (!project._id || !project._id._id) {
                    return null;
                }
                return Log.aggregate({
                    ...
                    var task, pTask, count = 0, pid, tpid;
                    for (var i = 0; i < tasks.length; i++) {
                        ...
                        for (var j = 0, plen = parentTasks.length; j < plen; j++) {
                            ...
                            if ( pid && tpid && tpid.toString() === pid.toString()) {
                                console.log('push task', task._id.name, pTask._id.name);
                                count++;
