'use strict';
var Q = require('q');
var _ = require('lodash');
var Moment = require('moment');
var ObjectId = require('mongoose').Types.ObjectId;
var SearchHelper = require('./searchHelper');
//models
var Task = require('../model/Task');
var Log = require('../model/log');
var Version = require('../model/version');
var Project = require('../model/project');



exports.query = function (options) {
    options = _.extend({ populate: true }, options);
    if (options.id) {
        return queryTaskById(options);
    } else {
        SearchHelper.preprocessQuery(options, ['projects', 'tags', 'peoples', 'classes', 'versions', 'tasks']);
        return SearchHelper.getIds(options, 'Id').then(function (idConditions) {
            return queryTasksByConditions(options, idConditions);
        });
    }
};

exports.count = function (options) {
    SearchHelper.preprocessQuery(options, ['projects', 'tags', 'peoples', 'classes', 'versions', 'tasks']);
    return SearchHelper.getIds(options, 'Id').then(function (idConditions) {
        return countTasksByConditions(options, idConditions);
    });
};


function countTasksByConditions(options, idConditions) {
    var deferred = Q.defer();
    var conditions = getQueryConditions(options, idConditions);
    Task.count(conditions, function (err, counts) {
        if (err) {
            return deferred.reject(err);
        }
        deferred.resolve({count:counts});
    });
    return deferred.promise;
}

function queryTasksByConditions(options, idConditions) {
    var deferred = Q.defer();
    var conditions = getQueryConditions(options, idConditions);
    //console.log(conditions);
    var queryOptions = getQueryOptions(options);

    function calculateTaskTimeConsume(task) {
        var deferred = Q.defer();
        var taskId;
        if (_.isString(task)) {
            taskId = new ObjectId(task);
        } else {
            taskId = new ObjectId(task._id || task);
        }
        Log.aggregate()
            .match({task: taskId})
            .group({_id: '$task', totalTime: {$sum: "$len"}})
            .exec()
            .then(function (result) {
                var calculteResult = result[0],
                    totalTime = 0;
                if (!calculteResult) {
                    totalTime = 0;
                } else {
                    totalTime = calculteResult.totalTime;
                }
                //if have children, then calculate the children's time consume
                if (task.childrenIds && task.childrenIds.length > 0) {
                    Q.allSettled(task.childrenIds.map(calculateTaskTimeConsume)).then(function (results) {
                        totalTime = results.reduce(function (totalTime, result) {
                            if (result.state === "fulfilled") {
                                var data = result.value;
                                task.children.some(function(childTask){
                                    if (childTask._id.toString() === data._id.toString()) {
                                        childTask.totalTime = data.totalTime;
                                    }
                                });
                                totalTime += data.totalTime;
                            }
                            return totalTime;
                        }, totalTime);
                        task.totalTime = totalTime;
                        deferred.resolve();
                    });
                } else {
                    task.totalTime = totalTime;
                    deferred.resolve({
                        totalTime: totalTime,
                        _id: taskId
                    });
                }
            }).on('reject', function (err) {
                deferred.reject(err);
            });
        return deferred.promise;
    }
    // var query = Task.find(conditions, null, queryOptions);
    //console.log(JSON.stringify(conditions), JSON.stringify(queryOptions));
    // if (options.populate) {
    //     query = query.populate([{
    //         path: 'children'
    //     }]);
    // }
    // var taskStatus = options.status;
    // var isMarked = (options.marked === 'true' || options.marked === true);
    // var withoutParent = (options.parent === undefined);
    var hierarchy;
    if (options.hierarchy) {
        hierarchy = parseInt(options.hierarchy, 10);
    }
    if (options.parent || hierarchy === 1) {
        Task.find(conditions, options.fields || null, queryOptions, function (err, tasks) {
            //calculate total time consume for each task
            if (options.calculateTimeConsume) {
                Q.allSettled(tasks.map(calculateTaskTimeConsume)).then(function () {
                    deferred.resolve(tasks);
                }).catch(function (err) {
                    deferred.reject(err);
                });
            } else {
                deferred.resolve(tasks);
            }
        });
        return deferred.promise;
    }

    Task.aggregate([{
        $match: conditions
    }, {
        $sort: {lastActiveTime: -1}
    }]).group({
        _id: '$parent',
        items: {$push: "$$ROOT"}
    }).exec(function (err, lists) {
        var tasks = [];
        //return deferred.resolve(lists);
        if (err) {
            return deferred.reject(err);
        }
        var parentIndex, parentList;
        lists.some(function (list, index) {
            if (list._id === null) {
                parentIndex = index;
                return true;
            }
            return false;
        });
        if (parentIndex !== undefined) {
            parentList = lists.splice(parentIndex, 1)[0];
        }
        if (parentList) {
            parentList.items.forEach(function (parentItem) {
                var parentId = parentItem._id.toString();
                var matchIndex;
                lists.some(function (list, index) {
                    var listId = list._id;
                    if (listId !== null) {
                        listId = listId.toString();
                    }
                    if (listId === parentId) {
                        matchIndex = index;
                        return true;
                    }
                    return false;
                });
                //console.log('calculate', matchIndex,parentItem.name, parentItem.children);
                if (matchIndex !== undefined) {
                    parentItem.childrenIds = parentItem.children;
                    parentItem.children = lists.splice(matchIndex, 1)[0].items;
                } else {
                    parentItem.childrenIds = parentItem.children;
                    parentItem.children = [];
                }
            });
            tasks = parentList.items;
        }
        tasks = lists.reduce(function (result, list) {
            return result.concat(list.items);
        }, tasks).filter(function (task) {
            if (options.status === 'done' && task.progress > 0 && task.progress < 100 && _.isEmpty(task.children)) {
                return false;
            } else {
                return true;
            }
        });
        //calculate total time consume for each task
        if (options.calculateTimeConsume) {
            Q.allSettled(tasks.map(calculateTaskTimeConsume)).then(function () {
                if (options.populateFields) {
                    populateFields(options.populateFields.split(','))
                        .then(function (tasks) {
                            deferred.resolve(tasks);
                        });
                } else {
                    deferred.resolve(tasks);
                }
            }).catch(function (err) {
                deferred.reject(err);
            });
        } else {
            if (options.populateFields) {
                populateFields(options.populateFields.split(','))
                    .then(function (tasks) {
                        deferred.resolve(tasks);
                    });
            } else {
                deferred.resolve(tasks);
            }
        }


        function populateFields(fields) {
            var populatePromise = Q(tasks);
            fields.forEach(function (item) {
                if (item === 'project') {
                    populatePromise = populatePromise.then(function() {
                        return populate(Project, 'projectId', tasks);
                    });
                } else if (item === 'version'){
                    populatePromise = populatePromise.then(function(tasks) {
                        return populate(Version, 'versionId', tasks);
                    });
                }
            });
            return populatePromise;
        }


        function populate(Model, field, result) {
            return Q.promise(function (resolve, reject) {
                if (!Model) {
                    return resolve(result);
                }
                Model.populate(result, {path: field}, function (err, result) {
                    if (err) {
                        return  reject(err);
                    }
                    resolve(result);
                });
            });
        }

        
        // if (_.isEmpty(tasks)) {
        //     return deferred.resolve([]);
        // }
        // tasks.forEach(function (task) {
        //     task.children = task.children.filter(function (childTask) {
        //         var bool = true;
        //         if (isMarked) {
        //             bool = bool && childTask.marked === true;
        //         }
        //         if (taskStatus === 'doing') {
        //             return  bool && childTask.progress > 0 && childTask.progress < 100;
        //         } else if (taskStatus === 'done') {
        //             return  bool && childTask.progress === 100;
        //         } else {
        //             return bool && true;
        //         }
        //     }).sort(function (a, b) {
        //         return new Date(b.lastActiveTime).getTime() - new Date(a.lastActiveTime).getTime();
        //     });
        // });

        // if (withoutParent) {
        //     console.log(tasks.length);
        //     tasks = tasks.filter(function (task) {
        //         var parent = task.parent;
        //         //task is not a subtask of the task in the `tasks`
        //         var result  = !parent || !tasks.some(function (task) {
        //             return task._id.toString() === parent.toString();
        //         });
        //         return result;
        //     });
        //     console.log(tasks.length);
        // }
    });

    return deferred.promise;
}

function queryTaskById(id) {

}


function getQueryConditions(options, idConditions) {
    var parentCondition;
    var tags = options.tags;
    if (options.parent === null || options.parent === 'null') {
        parentCondition =  {parent: { $type: 10 }};
    } else if (options.parent) {
        parentCondition = {parent: options.parent};
    }
    var conditions = [],
        timeCondition = [];
    if (!_.isEmpty(idConditions)) {
        conditions = conditions.concat(idConditions);
    }
    if (parentCondition) {
        conditions.push(parentCondition);
    }
    if (options.start && options.end) {
        timeCondition.push({
            createTime: {
                $gt: new Moment(options.start).startOf('day').toDate(),
                $lt: new Moment(options.end).endOf('day').toDate()
            }
        });

        timeCondition.push({
            lastActiveTime: {
                $gt: new Moment(options.start).startOf('day').toDate(),
                $lt: new Moment(options.end).endOf('day').toDate()
            }
        });

        timeCondition.push({
            $and: [{
                lastActiveTime: { $gte: new Moment(options.end).endOf('day').toDate() }
            }, {
                createTime: {$lte: new Moment(options.start).startOf('day').toDate() }
            }]
        });
    }
    var taskStatus = options.status;
    if (taskStatus === 'doing') {
        conditions.push({
            $or: [
                {progress: {$gte: 0, $lt: 100}}, //doing
                {progress: -1, children : {$not: {$size: 0}}} //have no progress but have children
            ]
        });
    } else if (taskStatus === 'done'){
        conditions.push({
            $or: [{
                progress: 100
            }, {
                progress: {$lt: 100},
                children : {$not: {$size: 0}}
            }]
        });
    }
    // if (options.projectId) {
    //     conditions.push({
    //         projectId: new ObjectId(options.projectId)
    //     });
    // }

    // if (options.versionId) {
    //     conditions.push({
    //         versionId: new ObjectId(options.versionId)
    //     });
    // }

    var isMarked = (options.marked === 'true' || options.marked === true);

    if (isMarked) {
        conditions.push({
            marked: isMarked
        });
    }

    if (options.name) {
        conditions.push({
            name: options.name
        });
    }

    if (!_.isEmpty(timeCondition)) {
        conditions.push({
            $or: timeCondition
        });
    }


    if (options.dueDays) {
        var dueDays = parseInt(options.dueDays, 10);
        //var now = new Moment();
        //console.log(dueDays, new Moment().add(dueDays, 'days').format('YYYY-MM-DD'));
        conditions.push({
            dueTime: {
                "$gt": new Moment().toDate(),
                "$lte": new Moment().add(dueDays, 'days').toDate()
            }
        });
    }
    if (options.overDue === 'true') {
        conditions.push({
            dueTime: {
                "$lt": new Moment().toDate()
            }
        });
    }

    if (options.notActiveDay) {
        var notActiveDay = parseInt(options.notActiveDay, 10);
        conditions.push({
            lastActiveTime: {
                "$lte": new Moment().subtract(notActiveDay, 'day').toDate()
            }
        });
    }

    if (!_.isEmpty(tags)) {
        conditions.push({
            tags: {
                $in: tags
            }
        });
    }

    if (!_.isEmpty(conditions)) {
        return {
            $and: conditions
        };
    }

    return null;
}

function getQueryOptions(userOptions) {
    var queryOptions = _.pick(userOptions, ['limit', 'skip']);
    //默认按照最后活动时间倒序排列
    queryOptions.sort = {lastActiveTime: -1};
    return queryOptions;
}
