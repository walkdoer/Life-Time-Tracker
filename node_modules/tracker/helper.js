...
var estimateRegexp = /!\[\s*(\d{1,2}\s*:\s*\d{1,2}\s*~?(\s*\d{1,2}\s*:\s*\d{1,2})?)\s*\]/;
...
function getSimpleTags(data) {
    //var result = data.match(/\[.*?\](?!\()/ig);
    var result = data.replace(estimateRegexp, '').match(/\[.*?\]/ig);
    var tags = [];
    if (!result) {
        return null;
    }
...
function getEstimate(str) {
    var timeStr, timeArr, res = null;
    var result = str.match(estimateRegexp);
    var todayStr = new moment().format('YYYY-MM-DD');
    var a, b;
    if (result && result[1]) {
        timeStr = result[1];
        timeArr = timeStr.split('~').map(function (item) { return item.trim();});
        res = {};
        var startTimeStr = timeArr[0];
        var endTimeStr = timeArr[1];
        if (startTimeStr && endTimeStr) {
            startTimeStr = fixStr(startTimeStr);
            endTimeStr = fixStr(endTimeStr);
            a = new moment(todayStr + ' ' + startTimeStr);
            b = new moment(todayStr + ' ' + endTimeStr);
            if (a.diff(b) > 0) {
                b.add(1, 'day');
            }
            res.start = a.toDate();
            res.end = b.toDate();
            res.diff = b.diff(a, 'minute');
...
function fixStr(dateStr) {
    return dateStr.split('~').map(function (ds) {
        return ds.trim().split(':').map(function (ts) {
            return ts.trim();
        }).join(':');
    }).join('~');
}
...
function getTimeSpanFromLog(log, config) {
    ...
    var timeSpan = null;
    var result = log.replace(estimateRegexp, '').match(timeSpanRegexp);
...
function getLogInfo(config) {
    try {
        ...
        var logInfo = {
            ...
            peoples: getPeoples(log),
            signs: getSigns(log),
            estimatedTime: getEstimatedTime(log),
            index: config.index,
            origin: log
        };
        var estimate = getEstimate(log);
        if (estimate) {
            logInfo.estimateStart = estimate.start;
            logInfo.estimateEnd = estimate.end;
        }
        if (config.noTime) {
            return logInfo;
        }
        var timeSpan = getTimeSpanFromLog(log, config);
        var estimatedTime = logInfo.estimatedTime;
        if (!estimatedTime) {
            if (logInfo.estimateStart && logInfo.estimateEnd) {
                logInfo.estimatedTime = new moment(logInfo.estimateEnd).diff(logInfo.estimateStart, 'minute');
            }
        }
...
function getLogContent(logStr) {
    ...
    tmp = tmp.replace(estimateRegexp, '')
...
function removeTimeString(logStr) {
    var estimateStr = logStr.match(estimateRegexp);
    var holder = '{{__HOLDER__}}';
    var str = logStr.replace(estimateRegexp, holder)
                .replace(/\d{1,2}\s*[:]\s*\d{1,2}\s*\s*[~～-]\s*(\d{1,2}\s*[:]\s*\d{1,2})*/ig, '');
    if (estimateStr) {
        str = str.replace(holder, estimateStr[0]);
    }
    return str;
}
...
function getPeoples(log) {
    var peoples = [];
    var peopleRegexp = /@\((.*?)\)/g;
    var peopleArr;
    while ((peopleArr = peopleRegexp.exec(log)) !== null) {
      peoples.push(peopleArr[1]);
    }
    return peoples;
}
...
function getSleepMoment(logs) {
    var sleepMoment;
    logs.some(function(log) {
        var isSleepLog = log.start === log.end && log.signs.indexOf('sleep') >= 0;
        if (isSleepLog) {
...
function isWakeLog(log, index) {
    ...
    if (origin.replace(estimateRegexp, '').match(timeSpanRegexpWithDoingLog) === null && index === 0) {
        return true;
    }
    return false;
}
...
function isSleepLog(log, index, lastIndex) {
    var origin = log.origin;
    if (origin.replace(estimateRegexp, '').match(timeSpanRegexpWithDoingLog) === null && index === lastIndex) {
        return true;
    }
    return false;
}
...
function getTimeStr(log) {
    var result = log.replace(estimateRegexp, '').match(/\d{1,2}\s*[:]\s*\d{1,2}\s*\s*[~～-]\s*(\d{1,2}\s*[:]\s*\d{1,2})*/ig);
    if (result && result.length >= 1) {
        return result[0];
    }
}

function covertLogtoString(log) {
    var task = log.task;
...
exports.getSleepMoment = getSleepMoment;
exports.covertLogtoString = covertLogtoString;