...
var logClassName = {};
LogClass.find({}, function (err, result) {
    if (err) {
        return console.log(err);
    }
    result.forEach(function (cls) {
        logClassName[cls._id] = cls.name;
    });
});
...
function getLogs(data, date, includeNoTimeLog, returnErrorMsg) {
    ...
    logStrArr.forEach(function(logStr, index) {
        ...
        if (startHour > 12 && endHour < 12 && hourSpan >= 8 && hourSpan <= 18) {
            ...
            warns.push(new WarnInfo(date, logStr, index, 'time is too many (' + hourSpan +' hours), maybe is not right'));
        }
        ...
        if (logInfo) {
            if (logInfo.len < 0) {
                ...
                errors.push(new ErrorInfo(date, logStr, index, 'start time is bigger then end time'));
            }
...
function BaseInfo(date, origin, index, message) {
    this.date = date;
    this.origin = origin;
    this.index = index;
...
function getTimeSpanFromLog(log, config) {
    ...
    var patchEnd = config.patchEnd === undefined ? true : config.patchEnd;
    var timeSpan = null;
...
function getLogSequenceError(logs) {
    ...
    logs.reduce(function(pv, cv) {
        ...
        if (cvStart.diff(pvEnd, 'minute') < 0) {
            ...
            errors.push(new ErrorInfo('', '', null, msg));
        }
        return cv;
    });
    return errors;
...
var getLogClassesFromDays = getItemFromDays('classes', function(item, log) {
    return extend({id: item}, {time: log.len});
